...
func main() {
	...
	var msg string
	//log.Println(printVer)
	flag.BoolVar(&printVer, "version", false, "print version")
	flag.StringVar(&configFile, "c", "configs.json", "specify config file")
	...
	flag.StringVar(&msg,"or","","a file need to break")
	flag.Parse()
	...
	if msg!=""{
		...
		msg_file:=ss.BreakMsg(msg)
		/*
		data,err := ioutil.ReadFile(string(msg_file))
		
		 fmt.Println("---------------------READING------------s-----------")
	    if err != nil {
	        return
	    }
		*/
	   ...
	   // fmt.Println(string(data[:]))
	    ...
	    err:= json.Unmarshal(msg_file, &cmdConfig)
	    //fmt.Println(cmdConfig)
	    if err != nil {
	        log.Println(err)
	        return
	    }
	...
	if configFile!=""{
		...
		configs,err := ss.ParseConfig(configFile)//*****************************CHECK
		...
		if !os.IsNotExist(err) {
			...
			ss.UpdateConfig(configs, &cmdConfig)
